...
public class Administrator implements Person {
	...
	private KWArrayList<String> messages;
	...
	public  void printBranches() {	
		for(int i=0;i<this.getCompany().getBranches().size();i++) {
				System.out.println(this.getCompany().getBranches().get(i));
	...
	public  void printEmployees() {	
		for(int i=0;i<this.getCompany().getEmployees().size();i++) {
				System.out.println(this.getCompany().getEmployees().get(i));
	...
	public boolean addBranche(String branchname) {
		...
		getCompany().getBranches().addLast(new Branches(branchname));
		return true;
	...
	public boolean removeBranche(String branchname)  {
			...
			if(getCompany().getBranches().remove(new Branches(branchname))) {
				for(int i=0;i<getCompany().getEmployees().size();i++) {
					if(getCompany().getEmployees().get(i).getInformation().equals(branchname)) {
						this.removeBranchEmployee(getCompany().getEmployees().get(i).getName(),getCompany().getEmployees().get(i).getSurname(),null);
					}
				}
				return true;
			}
	...
	public KWArrayList<String> getMessages() {
		return messages;
	}
